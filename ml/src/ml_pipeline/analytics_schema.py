...
class TechnologyCount(StrictContractModel):
    ...
    @classmethod
    def validate_canonical_skill(cls, value: str) -> str:
...
class AnalyticsSummary(StrictContractModel):
    ...
    def validate_cross_field_invariants(self) -> AnalyticsSummary:
        ...
        if technologies != sorted(
            technologies, key=lambda item: (-item.count, item.technology)
        ):
            raise ValueError(
                "top technologies must be ordered by count DESC and technology ASC"
            )
...
class PublishResponse(StrictContractModel):
